...
class Mpiigaze(Dataset):
    def __init__(self, pathorg, root, transform, train, angle, fold):
        self.transform = transform
        self.root = root
        self.orig_list_len = 0
        self.lines = []
        ...
        if isinstance(path, list):
            for i in path:
                with open(i) as f:
                    ...
                    self.orig_list_len += len(lines)
                    for line in lines:
                        patch_img = line.strip().split(" ")[0]

                        if os.path.isfile(os.path.join(self.root, patch_img)):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.isfile(os.path.join(self.root, face)):
            print(face)
            return
        else:
            img = Image.open(os.path.join(self.root, face))
